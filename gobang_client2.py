...
def main(sc, sk):
    ...
    while not again:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                sys.exit()
            if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 and round_this == 2 and winner == 0:  # 落子
...
if __name__ == '__main__':
    ...
    remote_ip, remote_port = waiting_screen(screen, socket)
    ready_screen(screen, remote_ip, remote_port, socket)
    main(screen, socket)
